...
1.  **Define a new template**: Use the `POST /api/v1/templates` endpoint to create a base definition for a notification type, such as a "welcome" for "email" notifications. This establishes a unique identifier and description for your template.
...
4.  **Retrieve active templates**: To fetch the content of the currently active template, use `GET /api/v1/templates/active` with query parameters like `code` for `template_code`, `lang` for `language_code`, and `type` for `notification_type`. This allows your notification system to always pull the most up-to-date and appropriate content for a specific template definition, language, and notification type.
